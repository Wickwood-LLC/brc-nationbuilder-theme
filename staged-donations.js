...
$(document).ready(function(){
  ...
  (function(){
    ...
    this.scrollContainerTop = function() {
      var progressStages = $('.progress-stages').eq(0);
      var navContainer = $('.nav-container').eq(0);
      if ($(window).width() > 768) {
        $('html, body').animate({
          scrollTop: progressStages.offset().top - navContainer.height() - 100
        }, 500);
      } else {
        $('html, body').animate({
          scrollTop: progressStages.offset().top - 100
        }, 500);
      }
    }
    ...
    this.updateProgressStages = function(stages, direction) {
      ...
      if (direction && current.next().length) { // direction = 0 if going backwards, 1 if going forwards
        current.hide('slide',{direction:'left',easing:'easeInBack'},300,function(){
          ...
          current.next().show('slide',{direction:'left'},300,function(){HeadlinerTheme.initForms($('.form label + .text'),$('input.quantity'),$('.form label + select'));$(this).addClass('active');StagedDonations.scrollContainerTop()});
        });
      } else if (!direction && current.prev().length) {
        current.hide('slide',{direction:'left',easing:'easeInBack'},function(){$(this).removeClass('active');
          current.prev().show('slide',{direction:'left'},300,function(){HeadlinerTheme.initForms($('.form label + .text'),$('input.quantity'),$('.form label + select'));$(this).addClass('active');StagedDonations.scrollContainerTop()});
        });
      } else {
        // do nothing
      }
  ...
  if ($('.progress-indicator-stages').length) {
    ...
    var amountSelect = $('.stage-1 > .radio-inline');
    var usStateDropdown = $('.us-only select[name="donation[billing_address_attributes][state]"]');
    ...
    $(window).load(function(){
      if (usStateDropdown.length) {
        var firstOption = usStateDropdown.children().eq(0);
        firstOption.attr('disabled','disabled').html('State');
      }
    });
